...
public partial class RCheckbox : IEntity
{
    ...
    [Parameter]
    public EventCallback<CheckboxEventArgs> OnClick { get; set; }
    ...
    private async Task check(MouseEventArgs e)
    {
        await this.toggleCheck(e);
        await InvokeAsync(() => StateHasChanged());
    }

    private async Task toggleCheck(MouseEventArgs e)
    {
        if (this.value == null)
            this.value = false;
        ...
        {
            var args = new CheckboxEventArgs() { Args = e, IsChecked = checkValue };
